                            ...
                            <div className="flex flex-col gap-2 bg-slate-900/10 dark:bg-slate-950/40 p-4 rounded-2xl border border-slate-850/50">
                                <textarea
                                    ...
                                    className={`min-h-24 w-full resize-y rounded-xl border p-3.5 text-sm outline-none transition-all focus:ring-2 focus:ring-purple-500/20 ${
                                        isDarkMode ? "border-slate-800 bg-slate-955 text-white" : "border-slate-200 bg-white text-slate-800"
                                    }`}
                                />
...
const ChatInterface = ({
    ...
    // Micro-batched token rendering state & callbacks
    const [displayedLastMessageText, setDisplayedLastMessageText] = useState("");
    ...
    useEffect(() => {
        ...
        if (isLastMessageAI && isLoading) {
            ...
            const updateText = () => {
                setDisplayedLastMessageText(targetText);
            ...
            if (timeSinceLastUpdate >= 32) { // Throttle rendering updates to ~30 FPS
            ...
            setDisplayedLastMessageText(lastMessage.text);
        }
    ...
    }, [lastMessage?.text, isLastMessageAI, isLoading]);
    ...
    const handleRateMessage = useCallback((id, type) => {
        setRatings(prev => ({
            ...prev,
        ...
        <div className={`flex min-h-0 h-full w-full flex-1 flex-col overflow-hidden bg-transparent ${activeTheme}`}>
                        ...
                        <div className="flex min-w-0 items-center gap-3">
                            <div className="relative shrink-0">
                                <div className={`flex h-9 w-9 items-center justify-center rounded-xl border transition-all duration-300 ${
                                    ...
                                    <Bot className={`h-4 w-4 ${isLoading ? "animate-pulse" : ""} ${isDarkMode ? "text-purple-400" : "text-purple-650"}`} />
                                </div>
                                {isLoading && (
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-transparent relative">
                ...
                <div ref={chatContainerRef} onScroll={handleScroll} data-lenis-prevent className="scrollbar-none min-h-0 flex-1 overflow-y-auto px-3 pt-5 pb-8 sm:px-5 sm:pt-6 relative z-10 flex flex-col will-change-transform">
                    ...
                    <div className="mx-auto w-full max-w-3xl space-y-8 px-3 pb-36 sm:px-5 relative z-10">
                        <AnimatePresence mode="popLayout">
                            {messages.map((msg, idx) => {
                                const isUser = msg.role === "user";
                                const messageId = msg._id || `msg-${idx}`;
                                const isLast = idx === messages.length - 1;
                                const textToRender = (isLast && !isUser && isLoading) ? displayedLastMessageText : msg.text;
                        ...
                        {isLoading && (
                            ...
                            >
                                <div className="flex w-full gap-4.5 justify-start items-start">
                                    ...
                                    <div className={`flex h-9 w-9 shrink-0 select-none items-center justify-center rounded-full border text-sm transition-all duration-300 ${
                                        isDarkMode
                                            ? "border-slate-855 bg-slate-900 text-purple-400"
                                            : "border-slate-155 bg-white text-purple-650 shadow-sm"
                                    }`}>
                                        <Bot className="h-4.5 w-4.5 animate-pulse text-purple-400" />
                                    </div>

                                    {/* Seamless Text-centric Shimmer Lines */}
                                    <div className="flex-1 max-w-lg space-y-3 pt-1.5">
                                        <div className="gemini-loader-line w-full" />
                                        <div className="gemini-loader-line w-5/6" style={{ animationDelay: "0.2s" }} />
                                        <div className="gemini-loader-line w-3/4" style={{ animationDelay: "0.4s" }} />
                                        
                                        <div className="flex items-center gap-1.5 pt-1.5">
                                            <span className={`text-[9px] font-black uppercase tracking-widest ${isDarkMode ? "text-purple-400" : "text-purple-650"}`}>
